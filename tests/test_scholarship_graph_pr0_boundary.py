...
def test_pr0_keeps_state_changing_and_paid_capabilities_disabled_by_default() -> None:
    settings = Settings(
        env="test",
    ...
    assert settings.catalogue_completeness_mode_enabled is True
    assert settings.catalogue_browser_fetching_enabled is True
