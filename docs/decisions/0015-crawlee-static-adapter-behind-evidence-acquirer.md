...
- Hard-require Crawlee in main dependencies: rejected; increases image size and
  couples all environments before the security adapter is proven.
- Point Crawlee's default HTTP client at the public internet without
  SafeSourceFetcher: rejected; violates the trust boundary.
- Delay any Crawlee mention until a full HttpClient is ready: rejected; the
  packaging and factory path should be reviewable early.
