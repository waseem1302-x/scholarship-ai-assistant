...
function deadlineTone(value) {
  if (!value) {
    return { className: "warn", label: "Deadline varies" };
  }
  const now = new Date();
  const deadline = new Date(value);
  const days = Math.ceil((deadline - now) / (1000 * 60 * 60 * 24));
  if (days < 0) {
    return { className: "danger", label: `Closed ${formatDate(value)}` };
  }
  if (days <= 30) {
    return { className: "warn", label: `${days} days left` };
  }
  return { className: "good", label: `Deadline ${formatDate(value)}` };
}
  ...
  return `
    <article class="card">
      ...
      <div class="card-meta">
        ...
        <span class="pill">${humanize(opportunity.degree_level)}</span>
        <span class="pill">${humanize(opportunity.funding_type)}</span>
        <span class="pill ${deadline.className}">${escapeHtml(deadline.label)}</span>
      </div>
      <p><strong>Funding:</strong> ${escapeHtml(opportunity.funding_summary)}</p>
      <p><strong>Evidence:</strong> verified ${formatDate(opportunity.last_verified_at)} from official source.</p>
      <div class="card-actions">
        <button class="button secondary" type="button" data-detail="${opportunity.id}">View details</button>
        <button class="ghost" type="button" data-save="${opportunity.id}">Save</button>
        <a class="ghost" href="${escapeAttribute(opportunity.official_source_url)}" target="_blank" rel="noreferrer">Official source</a>
      ...
      <div class="detail-grid">
        <div class="detail-block full">
          ...
          <div class="card-meta">
            ...
            <span class="pill">${humanize(opportunity.funding_type)}</span>
            <span class="pill ${deadline.className}">${escapeHtml(deadline.label)}</span>
            <span class="pill good">Verified source</span>
            <span class="pill">${humanize(opportunity.data_confidence)} confidence</span>
          </div>
          <p><strong>Provider:</strong> ${escapeHtml(opportunity.provider_name)}</p>
          <p><strong>University:</strong> ${escapeHtml(opportunity.university_name || "Not stated / multiple institutions")}</p>
          <p><strong>Important:</strong> Match scores and eligibility explanations are decision support, not admission or visa predictions.</p>
        </div>
        ${detailBlock("Funding package", [
          ["Tuition", opportunity.tuition_coverage],
          ["Monthly stipend", opportunity.monthly_stipend_amount ? `${opportunity.monthly_stipend_amount} ${opportunity.monthly_stipend_currency || ""}` : null],
        ...
        ${warningBlock("Eligibility warnings", opportunity.eligibility_warnings)}
        ${warningBlock("Curator notes", opportunity.notes ? [opportunity.notes] : [])}
        <div class="detail-block full">
          <h3>Official source evidence</h3>
...
function detailBlock(title, rows) {
  const body = rows
    .map(([label, value]) => `<li><strong>${escapeHtml(label)}:</strong> ${renderValue(value)}</li>`)
    .join("");
  return `<div class="detail-block"><h3>${escapeHtml(title)}</h3><ul class="field-list">${body}</ul></div>`;
}
...
function warningBlock(title, items) {
  ...
  return `
    <div class="detail-block full warning">
