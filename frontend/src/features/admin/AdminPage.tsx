...
function ReviewCard({ item, onChanged }: { item: ReviewQueueItem; onChanged: () => void }) {
  ...
  function submitReverification() {
    ...
    void run(() => reverifySource(item.opportunity.id, sourceId, notes, password), "Unable to reverify the source.", "Source reverified and the catalogue record was updated.");
  }
...
function AdminRecordList({ records }: { records: AdminOpportunity[] }) {
  if (!records.length) return <p className="admin-empty">No catalogue records match these filters.</p>;
