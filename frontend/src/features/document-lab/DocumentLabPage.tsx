...
type ApplicationDocument = { id: string; name: string; opportunityName?: string };
type ApplicationList = { items: { id: string; opportunity: { name: string }; documents: { id: string; name: string }[] }[] };
...
export function DocumentLabPage() {
  ...
  const [selectedAppDocs, setSelectedAppDocs] = useState<Record<string, string>>({});
  ...
  async function load(signal?: AbortSignal) {
    try {
      const [nextPolicy, nextAssets, applications] = await Promise.all([
        apiClient.request<Policy>("/document-lab/policy", { signal }),
        apiClient.request<Asset[]>("/document-lab/assets", { signal }),
        apiClient.request<ApplicationList>("/applications", { signal }),
      ]);
      setPolicy(nextPolicy);
      setAssets(nextAssets);
      const appDocs: ApplicationDocument[] = applications.items.flatMap((item) =>
      ...
      const histories = await Promise.all(
        nextAssets.flatMap((asset) =>
          asset.versions.map(async (version) => [version.id, await apiClient.request<Analysis[]>(`/document-lab/versions/${version.id}/analyses`, { signal })] as const),
        ),
      );
      setAnalyses(Object.fromEntries(histories));
    } catch (error) {
      if (!signal?.aborted) setNotice(errorText(error));
    }
  }
  ...
  async function link(versionDocId: string, version: Version) {
    if (!versionDocId) { setNotice("Choose an application document before linking a private version."); return; }
    ...
    try { await apiClient.request(`/document-lab/application-documents/${versionDocId}/link`, { method: "POST", body: JSON.stringify({ version_id: version.id, confirmed: true }) }); setNotice("Private version linked after your confirmation. No application status was changed."); } catch (error) { setNotice(errorText(error)); }
  }
  ...
  return (
    <main className="workspace-page page-width document-lab-page">
      <section className="workspace-intro">
        <p className="eyebrow">Private Documents</p>
        <h1>Editorial feedback, under your control.</h1>
        <p>Use CVs, resumes, statements of purpose, personal statements, and motivation letters only. Feedback is not an eligibility, admission, funding, visa, plagiarism, or authorship decision.</p>
      </section>
      {!policy ? (
        <p aria-live="polite">Loading private Documents settings…</p>
      ) : !policy.enabled ? (
        <section className="assistant-warning" role="alert">
          <strong>Documents are unavailable</strong>
          <p>This deployment has not enabled the private storage, scanner, and worker configuration.</p>
        </section>
      ) : (
        <>
          <section className="assistant-panel">
            <h2>Upload a private draft</h2>
            <p>PDF or DOCX only · up to 10 MB · up to {policy.max_pages} pages · up to {policy.max_extracted_characters.toLocaleString()} extracted characters. Password-protected files, archives, macros, OCR/image-only files, and spreadsheets are not supported.</p>
            <form onSubmit={upload} className="assistant-form">
              <label htmlFor="document-kind">Document type</label>
              <select id="document-kind" value={kind} onChange={(event) => setKind(event.target.value as DocumentKind)}>
                {Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
              </select>
              <label htmlFor="document-file">Private PDF or DOCX</label>
              <input id="document-file" type="file" accept=".pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(event: ChangeEvent<HTMLInputElement>) => setFile(event.target.files?.[0] ?? null)} required />
              <button className="button button-primary" disabled={busy}>{busy ? "Uploading…" : "Upload private draft"}</button>
            </form>
            <label className="assistant-opt-in">
              <input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} />
              I understand that, only after I request analysis, this version’s extracted text is sent to the configured AI provider. I can export or delete it, and feedback is editorial guidance only.
            </label>
            <div><button type="button" onClick={exportData}>Export Documents data</button></div>
            {notice ? <p className="form-success" role="status">{notice}</p> : null}
          </section>

          <section aria-label="Private document versions">
            <h2>Private drafts</h2>
            {!assets.length ? (
              <p>No private drafts yet.</p>
            ) : (
              assets.map((asset) => (
                <article className="assistant-answer" key={asset.id}>
                  <div className="assistant-answer-head">
                    <div>
                      <p className="eyebrow">{labels[asset.document_kind]}</p>
                      <h3>{asset.display_name}</h3>
                      <p>Retention ends {new Date(asset.retention_expires_at).toLocaleDateString()}.</p>
                    </div>
                    <button className="button-danger" type="button" onClick={() => remove(asset)}>Delete</button>
                  </div>
                  <div className="assistant-section">
                    <label>
                      Upload a new immutable version
                      <input type="file" accept=".pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(event) => setReplacementFiles((current) => ({ ...current, [asset.id]: event.target.files?.[0] ?? null }))} />
                    </label>
                    <button type="button" disabled={busy || !replacementFiles[asset.id]} onClick={() => uploadVersion(asset)}>Upload new version</button>
                  </div>
                  {asset.versions.map((version) => (
                    <div className="assistant-section" key={version.id}>
                      <h4>Version {version.version_number}</h4>
                      <p><strong>Status:</strong> {statusText(version.status)} · <strong>Safety scan:</strong> {statusText(version.scan_status)} · <strong>Extraction:</strong> {statusText(version.extraction_status)} · <strong>Key:</strong> {version.encryption_key_version}</p>
                      {version.rejection_code ? <p role="alert">This version cannot continue: {statusText(version.rejection_code)}.</p> : null}
                      {version.status === "failed" ? <button type="button" disabled={busy} onClick={() => retry(version)}>Retry private preparation</button> : null}
                      {version.status === "ready" ? (
                        <div>
                          <button type="button" disabled={busy} onClick={() => analyse(asset, version)}>Request editorial feedback</button>
                          <button type="button" onClick={() => refreshAnalysis(version)}>Refresh analysis history</button>
                          {applicationDocuments.length ? (
                            <div>
                              <label>
                                Link to application document
                                <select
                                  value={selectedAppDocs[version.id] ?? ""}
                                  onChange={(event) => setSelectedAppDocs((prev) => ({ ...prev, [version.id]: event.target.value }))}
                                >
                                  <option value="">Select a private application document</option>
                                  {applicationDocuments.map((item) => (
                                    <option key={item.id} value={item.id}>
                                      {item.opportunityName ? `${item.opportunityName} — ${item.name}` : item.name}
                                    </option>
                                  ))}
                                </select>
                              </label>
                              <button type="button" onClick={() => link(selectedAppDocs[version.id] ?? "", version)}>Confirm link</button>
                            </div>
                          ) : (
                            <p>To link this version, first add a document coordination record in an application workspace.</p>
                          )}
                        </div>
                      ) : null}
                      {analyses[version.id]?.map((analysis) => <FeedbackView analysis={analysis} key={analysis.id} />)}
                    </div>
                  ))}
                </article>
              ))
...
function FeedbackView({ analysis }: { analysis: Analysis }) {
  ...
  return (
    <section className="assistant-section" aria-live="polite">
      <h4>Editorial feedback</h4>
      <p><strong>Status:</strong> {statusText(analysis.status)} · <strong>Provider:</strong> {statusText(analysis.provider_status)}</p>
      ...
      {grouped.map(([category, items]) => (
        items.length ? (
          <div key={category}>
            <h5>{statusText(category)}s</h5>
            <ul>
              {items.map((item) => (
                <li key={item.id}>
                  {item.text}
                  <small> {statusText(item.rubric_category)} · {statusText(item.confidence)} confidence</small>
                  {item.excerpt ? <blockquote>“{item.excerpt}”</blockquote> : <small> General suggestion</small>}
                </li>
              ))}
            </ul>
          </div>
