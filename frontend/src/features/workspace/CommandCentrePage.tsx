...
export function CommandCentrePage() {
  ...
  const load = () => { setLoading(true); setError(null); void Promise.all([getApplications(), getCommandCentre(), getNotificationPreference()]).then(([items, dashboard, preferences]) => { setApplications(items); setCentre(dashboard); setRemindersEnabled(preferences.in_app_enabled); }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "Unable to load your application workspace.")).finally(() => setLoading(false)); };
