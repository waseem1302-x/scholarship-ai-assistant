...
function MatchCard({ match }: { match: OpportunityMatch }) {
  ...
  return <article className="match-card">
    <div className="match-card-header"><div><p className="eyebrow">{match.display_label}</p><h2>{match.opportunity.name}</h2><p>{match.opportunity.country} · {readableValue(match.opportunity.degree_level)} · {deadlineLabel(match.opportunity.application_deadline)}</p></div>
...
export function MatchesPage() {
  ...
  const { data, error: requestError, isLoading, reload } = useServerQuery<OpportunityMatch[]>(
    user?.id ?? "anonymous",
    (signal) => getMatches(signal),
    user?.role === "student",
  );
  const matches = data ?? [];
  const needsProfile = profileRequired(requestError);
  const error = requestError && !needsProfile
    ? requestError instanceof Error ? requestError.message : "Unable to load matches."
    : null;
