...
export function TrackerPage() {
  ...
  return <main className="workspace-tool page-width" aria-busy={isLoading}><section className="tool-header"><div><p className="eyebrow">Application tracker</p><h1>Turn research into a clear next step.</h1><p className="lead">Keep your status, personal deadline, and working notes together. This is private to your student account.</p></div><Link className="button button-quiet" to="/catalogue">Find scholarships</Link></section>
    ...
    {!isLoading && !error && !items.length ? <div className="catalogue-message"><h2>Your tracker is empty.</h2><p>Save a scholarship from its detail page when you are ready to start your application plan.</p><Link className="button button-primary" to="/catalogue">Explore verified scholarships</Link></div> : null}<div className="tracker-list">{items.map((item) => <TrackerCard key={item.id} item={item} onChanged={load} />)}</div>
  </main>;
}
